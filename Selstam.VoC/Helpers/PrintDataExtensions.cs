namespace Selstam.VoC.Helpers
{
    ...
    public static class PrintDataExtensions
    {
        ...
        private static void PrintColumns(string name, DataColumn[] columns)
        {
            ...
            WriteLine("{5}{0}{6}{1}{6}{2}{6}{3}{6}{4}{7}", horizontal0, horizontal1, horizontal2, horizontal3,
                horizontal4, Top_Left, Top_Center, Top_Right);
            ...
            WriteLine("{5}{0}{6}{1}{6}{2}{6}{3}{6}{4}{7}", horizontal0, horizontal1, horizontal2, horizontal3,
                horizontal4, Middle_Left, Middle_Center, Middle_Right);
            ...
            foreach (var column in columns)
            {
                ...
                var format2 =
                    string.Format(
                        "{{5}} {{0,{0}}} {{5}} {{1,-{1}}} {{5}} {{2,-{2}}} {{5}} {{3,-{3}}} {{5}} {{4}} {{1}} {{5,{4}}}",
                        length0, length1, length2, length3, length4 - dataMemberType.Length - column.ColumnName.Length);
                WriteLine(format2, column.Ordinal, column.ColumnName, column.DataType, column.AllowDBNull,
                    dataMemberType, Verticl_Bar);
            }

            WriteLine("{5}{0}{6}{1}{6}{2}{6}{3}{6}{4}{7}", horizontal0, horizontal1, horizontal2, horizontal3,
                horizontal4, Bottom_Left, Bottom_Center, Bottom_Right);
        ...
        public static void Print(this DataTable dataTable, bool rowOrdinals, ValueToStringHandler toString,
            params string[] columnNames)
        {
            Print(dataTable, rowOrdinals, 0, toString, columnNames);
        }

        public static void Print(this DataTable dataTable, int top, ValueToStringHandler toString,
            params string[] columnNames)
        {
            Print(dataTable, false, top, toString, columnNames);
        }

        public static void Print(this DataTable dataTable, bool rowOrdinals = false, int top = 0,
            ValueToStringHandler toString = null, params string[] columnNames)
        {
            PrintRows(dataTable, dataTable.Select(), rowOrdinals, top, toString, columnNames);
        }
        ...
        public static void Print(this DataView dataView, bool rowOrdinals, ValueToStringHandler toString,
            params string[] columnNames)
        {
            Print(dataView, rowOrdinals, 0, toString, columnNames);
        }

        public static void Print(this DataView dataView, int top, ValueToStringHandler toString,
            params string[] columnNames)
        {
            Print(dataView, false, top, toString, columnNames);
        }

        public static void Print(this DataView dataView, bool rowOrdinals = false, int top = 0,
            ValueToStringHandler toString = null, params string[] columnNames)
        {
        ...
        public static void Print(this DataSet dataSet, bool rowOrdinals, ValueToStringHandler toString,
            params string[] columnNames)
        {
            Print(dataSet, rowOrdinals, 0, toString, columnNames);
        }

        public static void Print(this DataSet dataSet, int top, ValueToStringHandler toString,
            params string[] columnNames)
        {
            Print(dataSet, false, top, toString, columnNames);
        }

        public static void Print(this DataSet dataSet, bool rowOrdinals = false, int top = 0,
            ValueToStringHandler toString = null, params string[] columnNames)
        {
            foreach (DataTable dataTable in dataSet.Tables)
        ...
        public static void Print(this DataRow[] dataRows, bool rowOrdinals, ValueToStringHandler toString,
            params string[] columnNames)
        {
            Print(dataRows, rowOrdinals, 0, toString, columnNames);
        }

        public static void Print(this DataRow[] dataRows, int top, ValueToStringHandler toString,
            params string[] columnNames)
        {
            Print(dataRows, false, top, toString, columnNames);
        }

        public static void Print(this DataRow[] dataRows, bool rowOrdinals = false, int top = 0,
            ValueToStringHandler toString = null, params string[] columnNames)
        {
            PrintRows(dataRows.Length != 0 ? dataRows[0].Table : null, dataRows, rowOrdinals, top, toString,
                columnNames);
        }
        ...
        private static void PrintRows(DataTable dataTable, IEnumerable<DataRow> dataRows, bool rowOrdinals, int top,
            ValueToStringHandler toString, string[] columnNames, string ordinalColumnName = null)
        {
            ...
            if (rowOrdinals)
                if (dataRows.Any())
                {
                    var maxRowOrdinal = string.IsNullOrEmpty(ordinalColumnName) ? dataRows.Select(row => row.Table.Rows.IndexOf(row)).Max() : dataRows.Select(row => (int) row[ordinalColumnName]).Max();
            ...
            foreach (var row in dataRows)
            {
                ...
                for (var i = 0; i < columns.Length; i++)
                {
                    ...
                    if (toString != null)
                    {
                        str = toString(obj, row, columns[i]) ?? "null";
                    }
                    else
                    {
        ...
        private static void PrintRows(DataView dataView, bool rowOrdinals, int top, ValueToStringHandler toString,
            string[] columnNames)
        {
            string ordinalColumnName = null;
            var dataTable = GetTableFromView(dataView, rowOrdinals, top, ref ordinalColumnName);
            PrintRows(dataTable, dataTable.Select(), rowOrdinals, top, toString, columnNames, ordinalColumnName);
        }

        private static DataTable GetTableFromView(DataView dataView, bool rowOrdinals, int top,
            ref string ordinalColumnName)
        {
            ...
            string ordinalColumnName = null)
        {
            if (columnNames != null && columnNames.Length > 0)
                return columnNames
                    .Join(dataTable.Columns.Cast<DataColumn>(), n => n, c => c.ColumnName, (n, c) => c,
                        StringComparer.CurrentCultureIgnoreCase).Where(c =>
                        string.IsNullOrEmpty(ordinalColumnName) || c.ColumnName != ordinalColumnName).ToArray();
            return dataTable.Columns.Cast<DataColumn>()
                .Where(c => string.IsNullOrEmpty(ordinalColumnName) || c.ColumnName != ordinalColumnName).ToArray();
        }

        private static void CalculateLengths(DataRow row, DataColumn[] columns, int[] lengths,
            ValueToStringHandler toString)
        {
            for (var i = 0; i < columns.Length; i++)
            {
                var obj = row[columns[i]];
        ...
        public static void PrintList(this DataTable dataTable, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataTable, false, 0, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataTable dataTable, bool rowOrdinals, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataTable, rowOrdinals, 0, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataTable dataTable, int top, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataTable, false, top, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataTable dataTable, int repeatColumns, RepeatDirection repeatDirection,
            params string[] columnNames)
        {
            PrintList(dataTable, false, 0, null, repeatColumns, repeatDirection, columnNames: columnNames);
        }

        public static void PrintList(this DataTable dataTable, bool rowOrdinals = false, int top = 0,
            ValueToStringHandler toString = null, int repeatColumns = 2,
            RepeatDirection repeatDirection = RepeatDirection.Vertical, string delimiter = ": ",
            params string[] columnNames)
        {
            PrintListRows(dataTable, dataTable.Select(), rowOrdinals, top, toString, repeatColumns, repeatDirection,
                delimiter, columnNames);
        }
        ...
        public static void PrintList(this DataView dataView, bool rowOrdinals, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataView, rowOrdinals, 0, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataView dataView, int top, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataView, false, top, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataView dataView, int repeatColumns, RepeatDirection repeatDirection,
            params string[] columnNames)
        {
            PrintList(dataView, false, 0, null, repeatColumns, repeatDirection, columnNames: columnNames);
        }

        public static void PrintList(this DataView dataView, bool rowOrdinals = false, int top = 0,
            ValueToStringHandler toString = null, int repeatColumns = 2,
            RepeatDirection repeatDirection = RepeatDirection.Vertical, string delimiter = ": ",
            params string[] columnNames)
        {
        ...
        public static void PrintList(this DataSet dataSet, bool rowOrdinals, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataSet, rowOrdinals, 0, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataSet dataSet, int top, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataSet, false, top, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataSet dataSet, int repeatColumns, RepeatDirection repeatDirection,
            params string[] columnNames)
        {
            PrintList(dataSet, false, 0, null, repeatColumns, repeatDirection, columnNames: columnNames);
        }

        public static void PrintList(this DataSet dataSet, bool rowOrdinals = false, int top = 0,
            ValueToStringHandler toString = null, int repeatColumns = 2,
            RepeatDirection repeatDirection = RepeatDirection.Vertical, string delimiter = ": ",
            params string[] columnNames)
        {
            foreach (DataTable dataTable in dataSet.Tables)
                PrintList(dataTable, rowOrdinals, top, toString, repeatColumns, repeatDirection, delimiter,
                    columnNames);
        }
        ...
        public static void PrintList(this DataRow[] dataRows, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataRows, false, 0, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataRow[] dataRows, bool rowOrdinals, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataRows, rowOrdinals, 0, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataRow[] dataRows, int top, ValueToStringHandler toString,
            params string[] columnNames)
        {
            PrintList(dataRows, false, top, toString, columnNames: columnNames);
        }

        public static void PrintList(this DataRow[] dataRows, int repeatColumns, RepeatDirection repeatDirection,
            params string[] columnNames)
        {
            PrintList(dataRows, false, 0, null, repeatColumns, repeatDirection, columnNames: columnNames);
        }

        public static void PrintList(this DataRow[] dataRows, bool rowOrdinals = false, int top = 0,
            ValueToStringHandler toString = null, int repeatColumns = 2,
            RepeatDirection repeatDirection = RepeatDirection.Vertical, string delimiter = ": ",
            params string[] columnNames)
        {
            PrintListRows(dataRows.Length != 0 ? dataRows[0].Table : null, dataRows, rowOrdinals, top, toString,
                repeatColumns, repeatDirection, delimiter, columnNames);
        }
        ...
        private static void PrintListRows(DataTable dataTable, IEnumerable<DataRow> dataRows, bool rowOrdinals, int top,
            ValueToStringHandler toString, int repeatColumns, RepeatDirection repeatDirection, string delimiter,
            string[] columnNames, string ordinalColumnName = null)
        {
            ...
            var cellFormat = string.Format(" {{{{0,-{0}}}}}{1}{{{{{{0}},-{2}}}}} {3}", columnsLength, delimiter,
                rowsLength, Verticl_Bar);
            ...
            switch (repeatDirection)
            {
                case RepeatDirection.Horizontal:
                    for (var i = 0; i < rowsCount; i++)
                    {
                        ...
                        PrintListRow(rows, columns, objects, i < rowsCount - 1 ? format : formatPartial, rowOrdinals,
                            toString, ordinalColumnName);
                    }

                    break;
                case RepeatDirection.Vertical when lastRowEmptyCellsCount == 0:
                    for (var i = 0; i < rowsCount; i++)
                    {
                        WriteLine(i == 0 ? header : separator);
                ...
                case RepeatDirection.Vertical when lastRowEmptyCellsCount > 0:
                    for (var i = 0; i < rowsCount; i++)
                    {
                        ...
                        var rows = dataRows.Where((r, n) =>
                            ...
                            n >= lastRowFilledCellsCount * rowsCount &&
                            (n - lastRowFilledCellsCount * rowsCount) % (rowsCount - 1) == i
                        );
                        PrintListRow(rows, columns, objects, i < rowsCount - 1 ? format : formatPartial, rowOrdinals,
                            toString, ordinalColumnName);
                    }
        ...
        private static void PrintListRows(DataView dataView, bool rowOrdinals, int top, ValueToStringHandler toString,
            int repeatColumns, RepeatDirection repeatDirection, string delimiter, string[] columnNames)
        {
            string ordinalColumnName = null;
            var dataTable = GetTableFromView(dataView, rowOrdinals, top, ref ordinalColumnName);
            PrintListRows(dataTable, dataTable.Select(), rowOrdinals, top, toString, repeatColumns, repeatDirection,
                delimiter, columnNames, ordinalColumnName);
        }

        private static void PrintListRow(IEnumerable<DataRow> rows, DataColumn[] columns, object[] objects,
            string format, bool rowOrdinals, ValueToStringHandler toString, string ordinalColumnName)
        {
            if (rowOrdinals)
            {
                var ordinals = string.IsNullOrEmpty(ordinalColumnName) ? rows.Select(row => row.Table.Rows.IndexOf(row)) : rows.Select(row => (int) row[ordinalColumnName]);
        ...
        private static void StreamWrite(Stream stream, Encoding encoding, string value = null, params object[] args)
        {
            ...
            var buffer = encoding.GetBytes(args == null ? value : string.Format(value, args));
        ...
        private static void StreamWriteLine(Stream stream, Encoding encoding, string value = null, params object[] args)
        {
            ...
            {
                buffer = encoding.GetBytes(args == null ? value : string.Format(value, args));
                stream.Write(buffer, 0, buffer.Length);
            }
